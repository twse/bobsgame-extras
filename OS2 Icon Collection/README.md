These textual icons are for use with OS/2 applications. The operating system should be at least OS/2 2.0 or newer.
...
To add or update existing software entries, attach the following text, with the fields enclosed by brackets "[]" replaced with your own identifying information. (Don't include the brackets!) The text should be enclosed in the appropriate comment syntax. The protocol for each download site should be http (add s to the protocol for secure connections). If the download site is an ftp server, no links are automatically created except in the Android GIT client OctoDroid. (The servers will appear underlined during editing.) For software without an OS/2 port, we recommend adding the words "no OS/2 port" after the software name in parentheses.
